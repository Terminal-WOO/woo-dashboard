...
import { createDocumentManagerBehavior, createSimulationBehavior } from './actors/DocumentManagerActor.js';
...
export async function createServer(pool: Pool) {
  ...
  fastify.post<{
    Params: { id: string },
    Body: { status: string }
  }>('/api/documents/:id/status', async (request, reply) => {
    const { id } = request.params;
  ...
  fastify.get<{ Params: { org: string } }>('/api/statistics/:org', async (request, reply) => {
    const org = request.params.org === 'utrecht'
      ? 'Gemeente Utrecht'
  ...
  // Get status history
  fastify.get<{ Params: { id: string } }>('/api/documents/:id/history', async (request, reply) => {
    const { id } = request.params;
