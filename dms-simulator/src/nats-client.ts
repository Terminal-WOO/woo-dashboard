...
export class NATSEventClient {
  ...
  async subscribe(callback: (event: DocumentEvent) => void): Promise<void> {
    ...
    const consumer = await this.js.consumers.get('DOCUMENT_EVENTS', 'document-event-consumer');
    const messages = await consumer.consume();

    (async () => {
      for await (const msg of messages) {
        try {
          const event = JSON.parse(new TextDecoder().decode(msg.data)) as DocumentEvent;
  ...
  getEventsBySystem(system: 'paperless' | 'alfresco', limit: number = 100): DocumentEvent[] {
    return this.events.filter(e => e.system === system).slice(0, limit);
  }

  getEventsByType(eventType: DocumentEvent['eventType'], limit: number = 100): DocumentEvent[] {
  ...
  getStats() {
    ...
    return {
      ...
      byType: {
        uploaded: this.events.filter(e => e.eventType === 'document.uploaded').length,
        updated: this.events.filter(e => e.eventType === 'document.updated').length,
        deleted: this.events.filter(e => e.eventType === 'document.deleted').length,
      },
    };
  }
